...
async function addNewAddressWhitelist(addr, db) {
  const response = await axios.get(
    "https://api.covalenthq.com/v1/1/address/" + addr + "/transactions_v2/",
    {
      params: {
  ...
  for (var address of addrs) {
    db.run(
      "INSERT INTO individual_recipient_whitelist (sender_address, recipient_address) VALUES (?, ?)",
      [addr, address]
    );
  }
}
  ...
  request(
    ...
    async function (error, response, body) {
      if (!error && response.statusCode == 200) {
        ...
        const events =
          body["transaction"]["transaction_info"]["call_trace"]["logs"];
